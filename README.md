...
### Установка и запуск сервера(брокера) сообщений
...
### Отправка сообщений с помощью программы "genelove"
...
### Проверка очереди сообщений на сервере
Для проверки пришло(ли) наше(и) сообщение(я) на сервер открываем адрес
http://localhost:8161/admin/queues
и в списке очередей ищем очередь с наименованием "inspection-queue". 
Проверяем число в первом столбце - это и есть количество сообщений, 
оно должно отличаться от значения до отправки.

### Получение сообщения с помощью программы "person-inspection"
