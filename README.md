...
### 1. Назначение DBService
    Данный сервис предназначен для облегчения работы с таблицами БД, ускорения разработки, обеспечения единой базы данных для команды разработчиков. Плюсы сервиса:
- быстрый доступ к таблицам БД без выхода из GeneLove
- легкий и интуитивно понятный интерфейс

### 2. Как пользоваться DBService
    Перед запуском проекта в файле DBServiceController.java отредактируйте строку 56. А именно - введите актуальный для вашей системы путь к каталогу "../genelove/src/main/resources/files/"
...
Выполнив эти не сложные щдействия - вы увидите список таблиц, существующих на сервере (на котором запущена GeneLove). Для каждой таблицы выводится количество записей в ней, и допустимые действия с данной таблицей.
...
- Clear/Reload - выполнить быструю перезагрузку данных в таблицу из файла. Это действие аналогично использованию следующих функций в приведенном порядке: Save, Clear, Load
- Clear - удалить все записи из таблицы (БУДЬТЕ ОСТОРОЖНЫ!)
- Delete - удалить таблицу (БУДЬТЕ ОСТОРОЖНЫ!)

### 3. Ограничения DBService
    DBService использует *.csv файлы, в которых, в качестве разделителя используется символ ";". Поэтому старайтесь избегать использования данного символа в данных таблицы, т.к. в этом случае DBService не сможет корректно загрузить данные из файла в таблицу
    Также, избегайте использование кириллицы в частности и символов Unicoded в общем, т.к. они имеют вид #xxxx; что опять же приветдет к тому, что DBService не сможет корректно загрузить данные из файла втаблицу
