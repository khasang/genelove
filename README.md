...
http://localhost:8080/admin/usersList .
...
Для проверки пришло(ли) наше(и) сообщение(я) на сервер открываем адрес 
http://localhost:8161/admin/queues ,
...
Клонируем исходники программы: 
https://github.com/DenisGuzikov/person-inspection.git
...
Запускаем программу. Теперь, чтобы проверить пришли ли сообщения открываем адрес: 
http://localhost:8081/allMessages
...
## DBService - встроенный доступ к таблицам на уровне разработчика
 
### 1. Назначение DBService
Данный сервис предназначен для облегчения работы с таблицами БД, ускорения
разработки, обеспечения единой базы данных для команды разработчиков. Плюсы сервиса:
- быстрый доступ к таблицам БД без выхода из GeneLove
- легкий и интуитивно понятный интерфейс

### 2. Как пользоваться DBService
Перед запуском проекта в файле DBServiceController.java отредактируйте строку 56.
А именно - введите актуальный для вашей системы путь к каталогу
"../genelove/src/main/resources/files/"
...
- Авторизуйтесь в Genelove
- Для использования сервиса DBService зайдите на страницу http://localhost:8080/DBService

Выполнив эти не сложные действия - вы увидите список таблиц, существующих на сервере
(на котором запущена GeneLove). Для каждой таблицы выводится количество записей в ней, и
 допустимые действия с данной таблицей.

На данный момент с каждой таблицей можно сделать следующее:
- View - просмотреть содержимое таблицы
- Save - сохранить содержимое таблицы в файл (в каталог ../genelove/src/main/resources/files/)
- Load - загрузить в таблицу данные из ранее сохранённого файла
- Clear/Reload - выполнить быструю перезагрузку данных в таблицу из файла. Это действие
аналогично использованию следующих функций в приведенном порядке: Save, Clear, Load.
- Clear - удалить все записи из таблицы (БУДЬТЕ ОСТОРОЖНЫ!)
- Delete - удалить таблицу (БУДЬТЕ ОСТОРОЖНЫ!)

### 3. Ограничения DBService
DBService использует *.csv файлы, в которых, в качестве разделителя используется
символ ";". Поэтому старайтесь избегать использования данного символа в данных таблицы,
т.к. в этом случае DBService не сможет корректно загрузить данные из файла в таблицу.
    Также, избегайте использование кириллицы в частности и символов Unicoded в общем,
т.к. они имеют вид #xxxx; что опять же приведёт к тому, что DBService не сможет
корректно загрузить данные из файла в таблицу.
     
### 4. Функционал DBService, планируемый к реализации
- Добавление записей в таблицу
- Редактирование записей таблицы
- Удаление записей из таблицы
- Из-за ограничений *.csv формата (точнее из-за использования в этом формате в качестве 
разделителя символа ";") перепишу функции сохранения и загрузки с использованием XML-формата
